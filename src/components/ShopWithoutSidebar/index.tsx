...
const ShopWithoutSidebar = () => {
  ...
  const [shopDataList, setShopDataList] = useState(shopData);
  const options = [
    { label: "Latest Products", value: "0" },
    { label: "Best Selling", value: "1" },
    { label: "Old Products", value: "2" }
  ];
  const prepareAssetItemUrls = (itemUrls, anchorAssetUrl, product) => {
    itemUrls.forEach((itemUrl) => {
    const _itemurl = "https://adn.nestortech.io/api/vi" + itemUrl.downloadUrl
      if (!product.imgs.previews.includes(_itemurl)) {
        product.imgs.previews.push(_itemurl);
      }
      if (!product.imgs.thumbnails.includes(_itemurl)) {
        product.imgs.thumbnails.push(_itemurl);
      }
      // if (itemUrl.downloadUrl != anchorAssetUrl) {
      //   itemUrlsList.push(
      //     "https://adn.nestortech.io/api/vi" + itemUrl.downloadUrl
      //   );
      // }
    });
    console.log(product, "after prepara")
  };
  const fetchProductImages = async () => {
    try {
      console.log(shopDataList, "shopData");
      for (const product of shopDataList) {
        // console.log(product.id);
        if (!["iphone", "watch"].includes(product.id as string)) {
          continue; // Skip products with id "iphone" or "watch"
        }
        console.log(product.id, "id");
        const response = await fetch(`/api/products/${product.id}`);
        const data = await response.json();
        if (data.errors) continue; // If there are errors in the response, do nothing
        console.log(data, product.id, "data for product id");
        // if (data.data.anchorAsset?.downloadUrl) {
        //   console.log(data, "data");
        //   // Check if downloadUrl is defined
        const anchorAssetUrl =
          "https://adn.nestortech.io/api/vi" +
          data.data.anchorAsset.downloadUrl;
        if (!product.imgs.thumbnails.includes(anchorAssetUrl)) {
          console.log(anchorAssetUrl);
          product.imgs.thumbnails.push(anchorAssetUrl);
          // product.imgs.previews.push(anchorAssetUrl);
        }
        if (!product.imgs.previews.includes(anchorAssetUrl)) {
          console.log(anchorAssetUrl);
          product.imgs.previews.push(anchorAssetUrl);
          // setProducts([...products, product]); // Update the products state when the image is loaded
        }
        //   console.log(anchorAssetUrl);
        //   product.imgs.thumbnails.push(anchorAssetUrl);
        //   product.imgs.previews.push(anchorAssetUrl);
        //   // setProducts([...products, product]); // Update the products state when the image is loaded
        // }
        prepareAssetItemUrls(data.data.assetItemUrls, anchorAssetUrl, product);
        // console.log("assetItemUrls: " , assetItemUrls)

        // product.imgs.thumbnails.push(...assetItemUrls);
        // product.imgs.previews.push(...assetItemUrls);
        setShopDataList([...shopDataList]);
      }
      // shopData.forEach(async (product) => {
      //   const response = await fetch(`/api/products/${product.id}`);
      //   const data = await response.json();
      //   if (data.errors) return; // If there are errors in the response, do nothing
      //   if (data.data.anchorAsset?.downloadUrl) {
      //     console.log(data, "data");
      //     // Check if downloadUrl is defined
      //     const anchorAssetUrl =
  ...
  useEffect(() => {
    ...
    console.log("useEffect Called", shopDataList);
  }, []);
